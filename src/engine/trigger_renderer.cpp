...
void TriggerRenderer::render(std::span<Ptr<Trigger const> const> triggers, vf::Frame const& frame) {
	auto circles = makeGroup(triggers.size());
...
std::span<vf::CircleShape> TriggerRenderer::makeGroup(std::size_t count) {
	m_circles.reserve(count);
	for (std::size_t i = m_circles.size(); i < count; ++i) { m_circles.push_back(vf::CircleShape(m_context->vfContext, "trigger")); }
