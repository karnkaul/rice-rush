...
bool setState(vf::Gamepad::Id const id, Controller::State& out, Controller::Flags& cache) {
...
bool setState(Keyboard const& keyboard, Controller::State& out, Controller::Flags& cache) {
...
Controller::State Controller::update(Keyboard const& keyboard) {
	...
	for (auto id = vf::Gamepad::Id{}; id < vf::Gamepad::max_id_v; ++id) {
		if (map.map[id] && setState(id, ret, m_cache)) {
	...
	if (setState(keyboard, ret, m_cache)) {
